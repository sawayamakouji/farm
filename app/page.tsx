...
import { AutoFeature } from './components/AutoFeature'

const CROP_TYPES = [
  { id: 'wheat', name: '小麦', baseValue: 10000, growthTime: 5, unlockCost: 0 },
  { id: 'corn', name: 'トウモロコシ', baseValue: 2, growthTime: 8, unlockCost: 1000 },
  { id: 'strawberry', name: 'イチゴ', baseValue: 5, growthTime: 15, unlockCost: 5000 },
]
...
export default function FarmClicker() {
      ...
      setPlayer(prev => {
        ...
        if (prev.unlockedFeatures.includes('autoSell') && prev.autoSell.enabled) {
          const totalValue = Object.entries(newState.crops).reduce((sum, [cropId, amount]) => {
            ...
            return sum + amount * (crop?.baseValue || 0)
          }, 0)
  ...
  const harvestCrop = (cropId: string) => {
    setPlayer(prev => ({
      ...
      crops: {
        ...prev.crops,
        [cropId]: (prev.crops[cropId] || 0) + prev.clickPower * prev.land
      }
    }))
  }
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          {CROP_TYPES.map(crop => (
            <motion.div
              ...
              onClick={(e) => {
                e.preventDefault();  // Prevent default to avoid double clicks
